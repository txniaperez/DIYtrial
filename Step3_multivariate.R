...
targets
group <- targets$group
group <- factor(group) #two levels of character levels
...
ggplot(pca.res.df) +
  ...
  #geom_label() +
  stat_ellipse() +
  xlab(paste0("PC1 (",pc.per[1],"%",")")) + #shows what PC1 means (disease status)
  ylab(paste0("PC2 (",pc.per[2],"%",")")) + #showing an outlier
  labs(title="PCA plot",
...
ggplot(pca.pivot) +
  ...
  facet_wrap(~PC) + #multiple factes of graph organized by PCs
...
mydata.filter <- mydata.df %>%
  dplyr::filter(geneID=="MMP1" | geneID=="GZMB" | geneID=="IL1B" | geneID=="GNLY" | geneID=="IFNG"
                | geneID=="CCL4" | geneID=="PRF1" | geneID=="APOBEC3A" | geneID=="UNC13A" ) %>% #looking for genes of interest
...
mydata.grep <- mydata.df %>%
  dplyr::filter(grepl('CXCL|IFI', geneID)) %>% #ooking for gene families
  dplyr::select(geneID, healthy.AVG, disease.AVG, LogFC) %>%
