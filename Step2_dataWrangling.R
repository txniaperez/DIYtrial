...
myTPM_gene <- Txi_gene$abundance #transcript per million metric
myCounts <- Txi_gene$counts #number of reads mapping to that gene
colSums(myTPM) #should be close to a million, none are a million because we moved from transcript to gene level data
colSums(myCounts) #total number of genes mapping from sample to reference
...
ggplot(myTPM.stats) + 
  ...
  #theme_bw() #cleans up graph
  #geom_hex(shape=16, size=2) #great for  visualizine overplotted data
...
ggplot(myTPM.stats) + 
  ...
  labs(y="Median", x = "Standard deviation",
       ...
       caption="DIYtranscriptomics - Spring 2023") +
  theme_classic() +
  theme_dark() + 
  theme_bw()
...
p1 <- ggplot(log2.cpm.df.pivot) +
  aes(x=samples, y=expression, fill=samples) + #fill means you want color to be different based on sample
  geom_violin(trim = FALSE, show.legend = FALSE) + #geom_violin is the shape you want
...
# take a look at this new DGEList object...how has it changed? normalization factor has changed because there has been internal normalization
...
p1 <- ggplot(log2.cpm.df.pivot) +
  ...
  labs(y="log2 expression", x = "sample",
       title="Log2 Counts per Million (CPM)",
       subtitle="unfiltered, non-normalized",
       caption=paste0("produced on ", Sys.time())) + #timestamping plots
  theme_bw()
