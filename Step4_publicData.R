...
dim(all.samples.mouse) #how many samples are in there
...
my.sample.locations <- which(all.samples.mouse %in% mySamples) # first time you've seen the %in% operator. lets us find of what samples in database matches our samples of interest
# extract gene symbols from the metadata
genes <- h5read(archs4.mouse, "meta/genes/gene_symbol") #no gene names?
...
keepers <- rowSums(archs4.cpm>1)>=2 #only keep sample that had cpm>1 in >=2 samples (duplicates)
archs4.dgelist.filtered <- archs4.dgelist[keepers,] #all columns for keepers rows
...
# let's try putting this all together in a study design file for only my samples
...
ggplot(pca.res.df) +
  aes(x=PC2, y=PC4,color=genotype) + #map different variable to color to see what is driving the variance
  geom_point(size=4) +
  # geom_label() +
  # stat_ellipse() +
...
ggplot(pca.pivot) +
  ...
  labs(title="PCA 'small multiples' plot",
       caption=paste0("produced on ", Sys.time())) +
  theme_bw() +
  coord_flip() #so sample names are easy to read
